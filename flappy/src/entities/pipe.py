...
class Pipes(Entity):
    ...
    def can_spawn_pipes(self) -> bool:
        ...
        return self.config.window.width - (last.x + last.w) > last.w * 5    # Change 2.5 to 5 for wider

    def spawn_new_pipes(self):
        # add new pipe when first pipe is about to touch left of screen
        self.gap_change -= 0.01
    ...
    def spawn_initial_pipes(self):
        ...
        upper_1.x = self.config.window.width - upper_1.w * 0.5
        lower_1.x = self.config.window.width - upper_1.w * 0.5
        self.upper.append(upper_1)
        self.lower.append(lower_1)

        upper_2, lower_2 = self.make_random_pipes()
        upper_2.x = upper_1.x + upper_1.w * (3.5 + 2.5)     # ADD 2.5
        lower_2.x = upper_1.x + upper_1.w * (3.5 +2.5)      # ADD 2.5
    ...
    def make_random_pipes(self):
        ...
        gap_y = random.randrange(0, int(base_y * 0.8 - self.pipe_gap))
        gap_y += int(base_y * 0.15)
        ...
        upper_pipe = Pipe(
            ...
            pipe_x,
            gap_y - pipe_height - self.gap_change,
        )
        ...
        lower_pipe = Pipe(
            ...
            pipe_x,
            gap_y + self.pipe_gap + self.gap_change,
        )
