...
class Beam:
    def __init__(self, x, y, index):
        self.image = [pygame.image.load("jetpack/assets/beam0.png").convert(),
                      pygame.image.load("jetpack/assets/beam2.png").convert(),
                      pygame.image.load("jetpack/assets/beam3.png").convert(),
                      pygame.image.load("jetpack/assets/beam4.png").convert()]
...
class Rocket:
    def __init__(self, x, y):
        self.image = [pygame.image.load("jetpack/assets/rocket_warning.png").convert(),
                      pygame.image.load("jetpack/assets/rocket.png").convert()]
        for image in self.image:
            image.set_colorkey((0, 0, 0))
            pygame.Surface.convert_alpha(image)
...
class Wall:
    def __init__(self, x, y, direction, speed_y, constants):
        self.image = [pygame.image.load("jetpack/assets/bottom.bmp").convert(),
                      pygame.image.load("jetpack/assets/top.bmp").convert()]
        for image in self.image:
            image.set_colorkey((0, 0, 0))
            pygame.Surface.convert_alpha(image)
...
class Laser:
    def __init__(self, x, y):
        self.image = [pygame.image.load("jetpack/assets/laser_off.bmp").convert(),
                      pygame.image.load("jetpack/assets/laser_on.bmp").convert()]
        for image in self.image:
            image.set_colorkey((0, 0, 0))
            pygame.Surface.convert_alpha(image)
        self.mask = pygame.mask.from_surface(self.image[1])
...
class Character:
    def __init__(self, x, y):
        self.image = [pygame.image.load("jetpack/assets/beggining.bmp").convert(),
                      pygame.image.load("jetpack/assets/flying.bmp").convert(),
                      pygame.image.load("jetpack/assets/dead.bmp").convert(),
                      pygame.image.load("jetpack/assets/walk1.bmp").convert(),
                      pygame.image.load("jetpack/assets/walk2.bmp").convert()]
        for image in self.image:
            image.set_colorkey((0, 0, 0))
            pygame.Surface.convert_alpha(image)
...
class Main_menu:
    def __init__(self):
        self.edge_pavement = pygame.image.load("jetpack/assets/edge_pavement.bmp").convert()
        self.main_pavement = pygame.image.load("jetpack/assets/main_pavement.bmp").convert()
        pygame.font.init()
        self.font = pygame.font.Font('jetpack/font.ttf',70)
    ...
    def update(self, screen, clock, constants):
        ...
        keys = check_input()
        if keys[K_SPACE] or keys[K_UP] or keys[K_DOWN] or keys[K_w]:
...
class Game(object):
    ...
    def update(self, screen, time_loop, test=False):
        keys = check_input()
        if (keys[K_UP] and not self.player.dead) or (keys[K_w] and not self.player.dead):
            self.player.up = 1
        if keys[K_ESCAPE]:
            self.done = True
        pygame.time.delay(1)
...
def main():
    ...
    while not done:
        ...
        score = game.counter.value
        Analysis.score_analysis(score, start_time)
...
username = Database.get_current_user()
score_list = []

class Analysis():
    # Function To Analyze Result Score
    @staticmethod
    def score_analysis(score, start_time):

        finish_time = datetime.now().strftime("%Y-%m-%d %H-%M-%S")
        accuracy = Analysis.analysis(score)
        ...
        # task = Thread(target=Database.update_score, args=(game, username, str(score), start_time, finish_time, accuracy,))
        # task.start()
        # Database.update_score(self.game, self.username, str(score), self.start_time, finish_time, self.accuracy)
        task = Thread(target=Database.insert_score, args=(username, game, score, start_time, finish_time, accuracy, "None"))
        task.start()
        print(score)
